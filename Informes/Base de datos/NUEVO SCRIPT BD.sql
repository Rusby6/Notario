...
CREATE TABLE PERSONA_FISICA (
    ...
    apelido_soltera VARCHAR(100),
    fecha_nacimiento DATE NOT NULL,
    estado_civil ENUM('soltero', 'casado', 'separado', 'viudo', 'divorciado') NOT NULL,
...
CREATE TABLE CONYUGE (
    id_conyuge INT PRIMARY KEY AUTO_INCREMENT,
    id_persona INT NOT NULL,
    nombre VARCHAR(100) NOT NULL,
    dni_nie VARCHAR(20) NOT NULL,
    regimen_matrimonial ENUM('gananciales', 'separacion', 'legal_pais') NOT NULL,
    fecha_creacion TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (id_persona) REFERENCES PERSONA_FISICA(id_persona) ON DELETE CASCADE
);

CREATE TABLE DOCUMENTOS_MATRIMONIO (
    id_documento INT PRIMARY KEY AUTO_INCREMENT,
    id_conyuge INT NOT NULL,
    tipo ENUM('capitulaciones', 'certificado_matrimonio', 'documento_separacion') NOT NULL,
    ruta_archivo VARCHAR(255) NOT NULL,
    fecha_creacion TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (id_conyuge) REFERENCES CONYUGE(id_conyuge) ON DELETE CASCADE
);
